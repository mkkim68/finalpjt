<template>
  ...
  <!-- Chatbot icon -->
  <div id="chatbot-icon" @click="toggleChat">
    💬
  </div>
  ...
  <div id="chatbot-window" :class="{ hidden: !chatVisible }">
    ...
    <div id="chatbot-input">
      <input id="chat-input" type="text" v-model="userInput" placeholder="메시지를 입력하세요..." @keyup.enter="sendMessageWithRateLimit">
...
const sendMessage = async () => {
  ...
  try {
    ...
    const errorMessage = {
      id: Date.now() + 2,
      text: error.response?.status === 429 ? "Error: Too many requests. Please wait a few seconds before trying again." : "Error: Could not get response from the chatbot.",
      type: "error"
    };
...
const sendMessageWithRateLimit = () => {
  ...
  if (now - lastRequestTime > REQUEST_INTERVAL) {
    ...
    const errorMessage = {
      id: Date.now(),
      text: `Too many requests. Please wait ${Math.ceil((REQUEST_INTERVAL - (now - lastRequestTime)) / 1000)} seconds before trying again.`,
      type: "error"
    };
    messages.value.push(errorMessage);
    nextTick(scrollToBottom);
